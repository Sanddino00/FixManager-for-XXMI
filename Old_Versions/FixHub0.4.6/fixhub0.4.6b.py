...
def run_script_thread(script_name):
    ...
    try:
        ...
        proc = subprocess.Popen(["python", script_name], cwd=target_folder.get())
        proc.wait()
...
def smooth_progress_bar(bar, target_value):
    """Animate progress bar smoothly towards target_value."""
    current = bar['value']
...
def download_resources_action():
    def thread_job():
        try:
            ...
            # Delete old resources & extract new
